candyjar = {
	...
	api : {
		...
		*/
		...
		classify : function( element, _class_ ){
			...
			if( candyjar.debugMode === true ){
				console.time( "candyjar.classify load time " );
			}
			...
			if( candyjar.debugMode === true ){
				console.timeEnd( "candyjar.classify load time " );
			}
		},
		...
		*/
		...
		evScroll : function( callback, throttle ){
			if( typeof throttle !== 'number' ){
				throttle = 10;
			}
			
			document.addEventListener( 'scroll' , function(){
				candyjar.api.evThrottle( callback, this, throttle );
			});
		},
	
		evResize : function( callback, throttle ){
			if( typeof throttle !== 'number' ){
				throttle = 100;
			}
		...
		evCSSanimationProperty : function( props ){
			var el = document.createElement('slipperydick');
...
/*
	*
	*	css animation instance constructor
	*
	*
	*
*/
...
candyAnimationInst = function( data ){
	this.ele = data.ele;
...
/*
	*
	*	css animation instance prototype
	*
	*
	*
*/

candyAnimationInst.prototype = {
	
	init : function(){
		
		var body = document.body, docElem = document.documentElement, scrollTop = window.pageYOffset || docElem.scrollTop || body.scrollTop, clientTop = docElem.clientTop || body.clientTop || 0;
		
		var rElePos = this.ele.getBoundingClientRect();
		this.top = rElePos.top +  scrollTop - clientTop;
		this.bottom = this.ele.offsetHeight + this.top;
		
		if( document.body.scrollTop < this.top - window.innerHeight || document.body.scrollTop > ( this.top + rElePos.height ) + window.innerHeight ){
			
			var animationOffset = 200, dTranslate = "-" + animationOffset + "px, 0";
			
			switch ( this.cardinal ){
				case 'left' : 
					dTranslate = "-" + animationOffset + "px, 0";
					break;
				case 'right' : 
					dTranslate = animationOffset + "px, 0";
					break;
				case 'top' : 
					dTranslate = "0, -" + animationOffset + "px";
					break;
				case 'bottom' : 
					dTranslate = "0, " + animationOffset + "px";
					break;
			...
			this.ele.style.transform = "translate( " + dTranslate + " )";
			this.ele.style.opacity = 0;
			this.ele.style.transition = "transform 1s ease-out, opacity 1s ease-out";
		}
...
/*
	*
	*	css animation handler function
	*
	*
	*
*/

candyAnimationHandler = {
	
	aniObjs : new Array(),
	aniThresholds : new Array(),
	
	init : function( eles ){
		for( var i=0, n=eles.length; i<n; i++ ){
			objInst = new candyAnimationInst( eles[i] );
			this.aniObjs.push( objInst );
			this.aniThresholds.push( objInst.top );
		}
		window.addEventListener( 'scroll', function(){
			candyjar.api.evThrottle( candyAnimationHandler.handler, this, 10 );
		});
		
		// console.log( this.aniObjs );
	},
	
	handler : function(){
		
		scrollPos = document.body.scrollTop || window.scrollY;
		
		var threshold = scrollPos + window.innerHeight;
		// console.log( threshold );
		
		for( var i = 0, n = candyAnimationHandler.aniThresholds.length; i<n; i++ ){
			// console.log( candyAnimationHandler.aniThresholds[i] );
			if( threshold > candyAnimationHandler.aniThresholds[i] ){
				candyAnimationHandler.dAnimate( candyAnimationHandler.aniThresholds[i] );
			}
		}
	},
	
	dAnimate : function( vPos ){
		
		for( var i = 0, n = candyAnimationHandler.aniObjs.length; i<n; i++ ){
			
			if( candyAnimationHandler.aniObjs[i].top === vPos){
				candyAnimationHandler.aniObjs[i].ele.style.transform = "translate( 0,0 )";
				candyAnimationHandler.aniObjs[i].ele.style.opacity = "1";
			}
		}
	}
}
...
candyjar.api.evScrollFunc = function( callback ){
	...
	function setup(){
		...
		animationLoop();
		// console.log( 'candyjar.api.evScrollFunc.setup' );
	};
	setup();
	...
	function mouseScroll( ev ){
		...
		if( ev.wheelDelta ){
			this.mouseDelta = ev.wheelDelta / 120;
		}
		else if( ev.detail ){
			this.mouseDelta = -ev.detail / 3;
		}
	};
...
candyjar.api.evLoad( function(){
	if( document.getElementsByClassName( 'page-id-320' ).length > 0 ){
		var _goo_aud = document.getElementById( 'welcome-to' );
		var _goo_aud_trigg = document.getElementById( 'welcome-to-trigger' );
	
		_goo_aud_trigg.addEventListener('click', function(){
			// console.log( 'yeah?' );
			_goo_aud.play();
			}
		);
